...
export function buildReceiptPdf(receipt: IReceipt, societyName: string): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.font(PDF_FONT).fontSize(9).fillColor('#dcfce7').text('Payment Receipt', M, 66);
      doc.font(PDF_FONT_BOLD).fontSize(20).fillColor('#fff').text('RECEIPT', M, 38, { width: right - M, align: 'right' });
      doc.font(PDF_FONT).fontSize(9).fillColor('#dcfce7')
      ...
      let y = 150;
      doc.font(PDF_FONT_BOLD).fontSize(9).fillColor(LIGHT).text('RECEIVED FROM', M, y);
      doc.font(PDF_FONT_BOLD).fontSize(12).fillColor(DARK).text(`Flat ${receipt.flatNumber}, ${receipt.blockName}`, M, y + 14);
      doc.font(PDF_FONT_BOLD).fontSize(9).fillColor(LIGHT).text('AMOUNT', 340, y, { width: right - 340, align: 'right' });
      doc.font(PDF_FONT_BOLD).fontSize(18).fillColor(GREEN).text(money(receipt.amountPaise), 340, y + 12, { width: right - 340, align: 'right' });

      y = 210;
      doc.roundedRect(M, y, right - M, 60, 10).fillAndStroke(SOFT, LINE);
      const cell = (label: string, value: string, x: number) => {
        doc.font(PDF_FONT).fontSize(8.5).fillColor(LIGHT).text(label, x, y + 12);
        doc.font(PDF_FONT_BOLD).fontSize(11).fillColor(DARK).text(value, x, y + 28);
      };
      ...
      if (receipt.allocations?.length) {
        doc.font(PDF_FONT_BOLD).fontSize(9).fillColor(LIGHT).text('APPLIED TO', M, y); y += 18;
        doc.rect(M, y, right - M, 22).fill(SOFT);
        doc.font(PDF_FONT_BOLD).fontSize(8.5).fillColor(GREY).text('INVOICE', M + 12, y + 7);
        doc.text('PERIOD', 320, y + 7); doc.text('AMOUNT', M, y + 7, { width: right - M - 12, align: 'right' });
        y += 22;
        doc.font(PDF_FONT).fontSize(9.5);
      ...
      if (receipt.advanceCreatedPaise > 0) {
        doc.font(PDF_FONT).fontSize(10).fillColor(GREEN).text(`Advance credit created: ${money(receipt.advanceCreatedPaise)}`, M + 12, y + 6);
        y += 20;
      }
