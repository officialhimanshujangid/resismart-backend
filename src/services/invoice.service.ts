  ...
  private static buildPdf(invoice: IInvoice, plan: IPlan, ctx: InvoiceContext): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.fillColor('#ffffff').font(PDF_FONT_BOLD).fontSize(26).text(appConfig.appName, M, 40);
        doc.font(PDF_FONT).fontSize(10).fillColor('#dbeafe').text('Society & Shop Management Platform', M, 72);
        doc.font(PDF_FONT_BOLD).fontSize(22).fillColor('#ffffff').text('INVOICE', M, 40, { width: right - M, align: 'right' });
        ...
        doc.fillColor('#ffffff').font(PDF_FONT_BOLD).fontSize(11).text('PAID', right - 92, 101, { width: 92, align: 'center' });

        // ── From / Billed To ──────────────────────────────────────────
        let y = 160;
        doc.font(PDF_FONT_BOLD).fontSize(9).fillColor(LIGHT).text('FROM', M, y);
        doc.font(PDF_FONT_BOLD).fontSize(12).fillColor(DARK).text(appConfig.appName, M, y + 14);
        doc.font(PDF_FONT).fontSize(9).fillColor(GREY).text(appConfig.supportEmail, M, y + 31);

        doc.font(PDF_FONT_BOLD).fontSize(9).fillColor(LIGHT).text('BILLED TO', 320, y);
        doc.font(PDF_FONT_BOLD).fontSize(12).fillColor(DARK).text(ctx.societyName || 'Society', 320, y + 14, { width: right - 320 });
        if (ctx.recipientEmail) doc.font(PDF_FONT).fontSize(9).fillColor(GREY).text(ctx.recipientEmail, 320, y + 31, { width: right - 320 });

        // ── Line items table ──────────────────────────────────────────
        y = 250;
        doc.rect(M, y, right - M, 26).fill(SOFT);
        doc.font(PDF_FONT_BOLD).fontSize(9).fillColor(GREY);
        doc.text('DESCRIPTION', M + 14, y + 9);
        doc.text('TENURE', 330, y + 9, { width: 90 });
        doc.text('AMOUNT', M, y + 9, { width: right - M - 14, align: 'right' });

        y += 26;
        doc.font(PDF_FONT_BOLD).fontSize(11).fillColor(DARK).text(`${plan.name || 'Plan'} Subscription`, M + 14, y + 12, { width: 300 });
        doc.font(PDF_FONT).fontSize(11).fillColor(DARK).text(String(ctx.tenure || 'Monthly'), 330, y + 12, { width: 90 });
        doc.text(money(planPrice), M, y + 12, { width: right - M - 14, align: 'right' });
        if (plan.description) doc.font(PDF_FONT).fontSize(8).fillColor(LIGHT).text(plan.description, M + 14, y + 30, { width: 300 });
        ...
        if (credit > 0) {
          doc.font(PDF_FONT).fontSize(10).fillColor(GREY).text('Subtotal', labelX, ty);
        ...
        doc.font(PDF_FONT_BOLD).fontSize(13).fillColor(DARK).text('Total Paid', labelX + 12, ty + 10);
        ...
        doc.font(PDF_FONT_BOLD).fontSize(9).fillColor(LIGHT).text('PAYMENT DETAILS', M + 16, pd + 14);
        const row = (label: string, value: string, idx: number) => {
          const ry = pd + 32 + idx * 16;
          doc.font(PDF_FONT).fontSize(10).fillColor(GREY).text(label, M + 16, ry, { width: 130 });
          doc.font(PDF_FONT_BOLD).fontSize(10).fillColor(DARK).text(value, M + 150, ry);
        };
        ...
        doc.font(PDF_FONT_BOLD).fontSize(8.5).fillColor(GREY).text(`Thank you for choosing ${appConfig.appName}.`, M, 775, { width: right - M, align: 'center' });
