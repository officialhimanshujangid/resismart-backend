    ...
    if (ownerIdentityIds.length) {
      ...
      for (const uid of ownerIdentityIds) {
        newResident = new Resident({
          ...
          societyId: new mongoose.Types.ObjectId(societyId),
          userId: uid,
          person: { name: validatedData.ownerName, email: validatedData.ownerEmail?.toLowerCase(), phone: validatedData.ownerPhone },
          relationship: 'OWNER',
          isOwner: true,
          isHead: true,
          isActive: true,
          ...
          if (ownerIdentityIds.length) {
            for (const uid of ownerIdentityIds) {
              newResident = new Resident({
                ...
                societyId: new mongoose.Types.ObjectId(societyId),
                userId: uid,
                person: { name: validated.ownerName, email: validated.ownerEmail?.toLowerCase(), phone: validated.ownerPhone },
                relationship: 'OWNER',
                isOwner: true,
                isHead: true,
                isActive: true,
